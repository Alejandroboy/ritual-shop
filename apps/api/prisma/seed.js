...
const BACKGROUNDS = [...Array.from({ length: 36 }, (_, i) => i + 1), 100, 200];
function familyRectCermet(codePrefix, labelPrefix) {
  return {
    ...
    sizeCodesByOrientation: {
      ...
      [client_1.Orientation.HORIZONTAL]: SIZES.RECT_CERMET.map((s) =>
  ...
  return {
    ...
    sizeCodesByOrientation: {
      ...
      [client_1.Orientation.HORIZONTAL]: [
        ...SIZES.RECT_SMALL,
        ...SIZES.RECT_TALL,
      ].map((s) => s.split('x').reverse().join('x')),
    },
    backgroundCodes: BACKGROUNDS,
  };
}
...
function familyRectGlass(codePrefix, labelPrefix) {
  return {
    ...
    sizeCodesByOrientation: {
      ...
      [client_1.Orientation.HORIZONTAL]: SIZES.RECT_SMALL.map((s) =>
        s.split('x').reverse().join('x'),
      ),
    },
    backgroundCodes: BACKGROUNDS,
...
function familyGrowth(codePrefix, labelPrefix) {
  return {
    ...
    sizeCodesByOrientation: {
      ...
      [client_1.Orientation.HORIZONTAL]: SIZES.GROWTH.map((s) =>
        s.split('x').reverse().join('x'),
      ),
    },
  };
  ...
  if (orientations.length) {
    for (const orient of orientations) {
      for (const cm of f.colorModes) {
        const suffix =
          `${f.shape === client_1.Shape.OVAL ? 'OV' : f.shape === client_1.Shape.ARCH ? 'AR' : 'R'}-` +
